...
using namespace std;
  ...
  Valor::Valor(double _valor, int _fila, int _columna){
    valor = _valor;
    fila = _fila;
    columna = _columna;
  };
  ...
  void MatrizDispersa::CreaMatriz_Dinamica(int F, int C){
    matriz = new double *[F];
    for(int i = 0; i < F; i++){
  ...
  void MatrizDispersa::RellenaMatriz(){
    int tam = 0, tam1 = 0;
  ...
  void MatrizDispersa::Muestra_Valores(){
    cout << "VALORES" << endl;
    for(int i = 0; i < numeroValores; i++){
  ...
  void MatrizDispersa::Muestra_Matriz(){
    cout << "MATRIZ" << endl;
    for(int i = 0; i < nfilas; i++){
      for(int j = 0; j < ncolumnas; j++){
        cout << "[" << matriz[i][j] << "]";
  ...
  void MatrizDispersa::Limpiar_Pantalla(){
